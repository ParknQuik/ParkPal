...
description: Monitor GCP infrastructure costs and resource usage for ParkPal using GCloud MCP
---
...
1. Use `mcp__gcloud__run_gcloud_command` to list Cloud Run services with resource specs
2. Use `mcp__gcloud__run_gcloud_command` to get Cloud SQL instance configuration
3. Use `mcp__gcloud__run_gcloud_command` to list storage buckets
4. Use `mcp__gcloud__run_gcloud_command` to check billing budgets
...
mcp__gcloud__run_gcloud_command({
  args: [
    ...
    "--format=json(metadata.name,spec.template.spec.containers[0].resources.limits)"
  ]
})
```
...
mcp__gcloud__run_gcloud_command({
  args: [
    "sql", "instances", "describe", "parkpal-db",
    "--project=parkpal-474417",
    "--format=json(settings.tier,settings.dataDiskSizeGb,state,settings.activationPolicy)"
  ]
})
```
...
```javascript
mcp__gcloud__run_gcloud_command({
  args: ["storage", "buckets", "list", "--project=parkpal-474417", "--format=json"]
})
```
...
```javascript
mcp__gcloud__run_gcloud_command({
  args: ["billing", "budgets", "list", "--billing-account=0127AF-2BABFA-97A08D", "--format=json"]
})
```
